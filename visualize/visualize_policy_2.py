...
# Instantiate DualQL (reset() builds initial policy)
ctrl = QLearning(
    cell_size=cell_size,
    env_size=env_size,
...
# Collect quiver data from DualQL policy which only uses (i, j) as state
X, Y, U, V = [], [], [], []
for (i, j), act in ctrl.policy.items():
    if grid[j, i] == 1:
        continue
...
ax.set_title("DualQL Initial Policy with Map Overlay")
