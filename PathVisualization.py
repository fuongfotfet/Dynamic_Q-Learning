...
def load_path(path):
    # Allow user to choose fixed runs for each algorithm
    print("Choose run number (1-20) for each algorithm:")
    run = []
    for i in range(len(index_algorithm)):
        while True:
            try:
                run_num = int(input(f"Enter run for {index_algorithm[i]} (1-20): "))
                if 1 <= run_num <= 20:
                    run.append(run_num)
                    break
                else:
    ...
    for i in range(len(index_algorithm)):
        with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "result", scenario, scenario + input_map, index_algorithm[i], "path.txt"), "r") as f:
...
if __name__ == "__main__":
    ...
    nums_of_section = int(input("Enter number of sections (1-4): "))
